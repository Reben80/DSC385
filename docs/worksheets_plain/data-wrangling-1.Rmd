...
```{r filter-species-island, eval = FALSE}
...
```{r select-remove, eval = FALSE}

```

And now remove both `species` and `island`.

```{r select-remove2, eval = FALSE}
...
```{r arrange-bill-length, eval = FALSE}
...
```{r arrange-bill-length-desc, eval = FALSE}
...
```{r mutate-bill-ratio, eval = FALSE}
penguins %>%
...
```{r flipper-length-count, eval = FALSE}
penguins %>%
  mutate(
    ___
  ) %>%
